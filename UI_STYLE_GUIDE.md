...
The infrastructure distribution chart uses clean stacked columns with restrained rounded corners, light horizontal gridlines, and a compact legend. Use the new signal palette for chart series: electric blue `#2F80ED`, violet `#8B5CF6`, coral `#F43F5E`, mint `#00BFA6`, and amber `#F2B84B`. Preserve real stacked values, hover/tooltips, responsive sizing, and CSV export. Do not replace chart data with a static image or decorative chart illustration.
